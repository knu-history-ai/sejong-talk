...
- 초기화/교체가 완료된 이전 쿠키의 늦은 요청은 401 SESSION_EXPIRED로 끝나며 쿠키를 발급하거나 지우지 않는다. 요청 도착 순서와 관계없이 같은 이전 쿠키로 세션을 여러 개 만들 수 없다. 자연 만료된 쿠키로 새 대화를 시작하는 요청은 한 번 허용한다. 쿠키가 없는 최초 시작의 중복 클릭은 화면에서 직렬화한다.
...
| 재시도 보호 | 사용을 마친 쿠키의 해시만 최대 4시간 보관. 1,000개가 쌓이면 새 생성/교체를 503으로 막고 만료 후 재개. 기존 세션 삭제는 가능하며 최대 활성 세션 수만큼 해시가 추가될 수 있음 |
...
`.env.local`의 `APP_ORIGIN`은 `https://도메인`처럼 끝의 / 없이 입력한다. 배포 실행에서 누락/잘못된 값은 503으로 차단하며 Host나 X-Forwarded-Host로 대체하지 않는다. APP_ORIGIN이 없는 로컬 `npm run dev`에서만 loopback Host(localhost·127.0.0.1·[::1])를 자동 인식한다. 서로 다른 호스트나 포트의 Origin은 여전히 차단한다. `npm start`로 로컬 시험할 때도 APP_ORIGIN을 명시해야 한다.
